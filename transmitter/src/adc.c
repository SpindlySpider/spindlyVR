...
K_SEM_DEFINE(adc_radio_sync_semaphore, 0, 1);
...
int config_saadc() {
  ...
  // init buffer
  // nrfx_saadc_buffer_set(sample_buf, CONFIG_TX_SAMPLE_NUMBER);
...
int config_timer() {
  ...
  // need 100 ticks to get 160khz (default ADC value) from 16mhz e.g. 16,000,000
  // get khz
  uint32_t desired_ticks = frequency / (CONFIG_TX_ADC_FREQUENCY * 1000);
...
int config_ppi() {
  ...
  // timestamp when ADC has finished taking readings
  err = nrfx_gppi_conn_alloc(
      nrf_saadc_event_address_get(NRF_SAADC, NRF_SAADC_EVENT_STARTED),
      nrf_timer_task_address_get(NRF_TIMER4, NRF_TIMER_TASK_CAPTURE3),
      &gppi_adc_timestamp);
  if (err != 0) {
    return err;
  }
...
int tx_matched_filter(struct cached_sin_cos_t *cached_s_c) {
  ...
  for (int i = 0; i < CONFIG_TX_SAMPLE_NUMBER; i++) {
    float ac_wave_signal = (float)sample_buf[i] - true_dc_offset;
