...
static float cal_magnetic_moment = 10000.0f;
...
void calculate_pos(float bx, float by, float bz) {
  ...
  // print values
  printk(" X: %-10.8f | Y: %-10.8f | Z: %-10.8f\n", xp, yp, zp);
}
...
void start_positioning_thread(void *, void *, void *) {
  ...
  while (1) {
    ...
    struct vector_t rx_pos_raw = {
        fabsf(incoming_data.bx), fabsf(incoming_data.by), fabsf(incoming_data.bz)};
    ...
    rot_vector = rotate_vector(rx_pos_raw, q_tx_frame, q_rx_frame);

    // printk(" ROT X: %-7.3f | ROT Y: %-7.3f | ROT Z: %-7.3f\n", rot_vector.x,
    //        rot_vector.y, rot_vector.z);

    // estimate position from rotated vector
    calculate_pos(rot_vector.x, rot_vector.y, rot_vector.z);
  }
}
