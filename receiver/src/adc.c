...
int16_t sample_buf_x[CONFIG_RX_SAMPLE_NUMBER];
int16_t sample_buf_y[CONFIG_RX_SAMPLE_NUMBER];
int16_t sample_buf_z[CONFIG_RX_SAMPLE_NUMBER];
...
int config_saadc() {
  ...
  // init buffer
  nrfx_saadc_buffer_set(sample_buf_x, CONFIG_RX_SAMPLE_NUMBER);
  nrfx_saadc_buffer_set(sample_buf_y, CONFIG_RX_SAMPLE_NUMBER);
  nrfx_saadc_buffer_set(sample_buf_z, CONFIG_RX_SAMPLE_NUMBER);
...
void switch_channel(nrf_saadc_input_t pin) {
  nrfx_saadc_channel_t channel_config = NRFX_SAADC_DEFAULT_CHANNEL_SE(pin, 0);
  channel_config.channel_config.acq_time = NRF_SAADC_ACQTIME_3US;
  // get / adjust gain here
  struct rx_data_container_t data;
  read_rx_data(&data);
  channel_config.channel_config.gain = data.gain;
}

void adc_sample(int16_t *buf, nrf_saadc_input_t pin) {
  // switch channel
  switch_channel(pin);
  // collect samples
...
void start_adc_thread(void *, void *, void *) {
  ...
  while (1) {
    ...
    adc_sample(&sample_buf_x,  PIN_X);
    adc_sample(&sample_buf_y,  PIN_Y);
    adc_sample(&sample_buf_z,  PIN_Z);

    tx_matched_filter(current_buffer_ptr, &cached_s_c);

    // TODO: update gain based on average value -> increase if below a certain value, decrease if too high?
